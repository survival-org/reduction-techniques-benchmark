...
simulate_correlated_variables <- function(n_variables, n_samples) {
  ...
  diag(cor_mat) <- 1
  cor_mat[upper.tri(cor_mat)] <- seq(
    0.1,
    0.5,
    length.out = sum(upper.tri(cor_mat))
  )
...
form <- ~ -3.5 +
  f0(t) +
  (v2 > 0) * (v1 < 0) * (f0(t) * (t < 4) + (t > 4) * 0) +
  .2 * v3 -
  .4 * v4

set.seed(24032018)
sim_df <- sim_pexp(form, as.data.frame(df), 1:10) |>
  select(-id)

saveRDS(sim_df, here::here("datasets/synthetic-breakpoint.rds"))
